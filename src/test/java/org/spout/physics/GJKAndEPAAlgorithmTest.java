...
public class GJKAndEPAAlgorithmTest {
	...
	private static void collideShapes(CollisionShape s1, Transform t1, CollisionShape s2, Transform t2) {
		...
		surface = t2.getOrientation().toMatrix3().transform(surface);
		t2.setPosition(in.sub(surface.mul(0.95f)));
	}
	...
	private static BroadPhasePair pair(CollisionShape s1, Transform t1, CollisionShape s2, Transform t2) {
		final CollisionBody b1 = new MobileRigidBody(t1, 0, Matrix3.IDENTITY, s1, ID++);
		final CollisionBody b2 = new MobileRigidBody(t2, 0, Matrix3.IDENTITY, s2, ID++);
		b1.updateAABB();
		b2.updateAABB();
